...
O3 in the upper troposphere and compare this to UT O3 obtained if
simply average the O3 mixing ratios from the model over the same
...
.. code-block:: bash
    ...
    usage: ut_o3_gc_test.py [-h] [--gc_dir GC_DIR] [--out_dir OUT_DIR]
...
from uptrop.cloud_slice_ozone import cldslice, CLOUD_SLICE_ERROR_ENUM
...
class ProcessedData:
    ...
    def __init__(self, region, str_res, strat_thld, do_temperature_correction=False, do_cld_frac_filter=False, do_cld_hght_test=False):
        ...
        self.g_o3_vcd = np.zeros(grid_shape)
        self.g_o3_vmr = np.zeros(grid_shape)
        ...
        self.true_o3 = np.zeros(grid_shape)  # "true" cloudy UT O3
        self.g_askut_o3 = np.zeros(grid_shape)  # "true" all-sky UT O3
        self.g_ask_gaus_wgt = np.zeros(grid_shape)  # "true" all-sky UT O3 weights
        self.g_ask_cnt = np.zeros(grid_shape)  # Count all-sky
        ...
        self.grad_retain = 0
        self.grad_remove = 0
        # Print out min and max cloud-sliced UT O3 error size:
        ...
        # Define factor to convert slope of O3 mixing ratio versus pressure
        # to VMR:
    ...
    def process_geoschem_day(self, file_path):
        ...
        self.strat_o3 = [[[] for n in range(self.ydim)] for m in range(self.xdim)]
        self.all_cld_p = [[[] for n in range(self.ydim)] for m in range(self.xdim)]
        self.g_true_o3 = [[[] for n in range(self.ydim)] for m in range(self.xdim)]
        ...
        for y in range(len(this_geoschem_day.t_lat)):
            for x in range(len(this_geoschem_day.t_lon)):
                this_geoschem_day.prepare_o3_pixel(x, y)
                self.regrid_and_process(x, y, this_geoschem_day)
        for i in range(self.xdim):
            for j in range(self.ydim):
                if any(value != 0 for value in self.g_o3[i][j]):
    ...
    def regrid_and_process(self, x, y, gc_data):
        ...
        self.g_askut_o3[lon, lat] += np.sum(gc_data.t_gc_o3[gc_data.askind, y, x] * gc_data.twgt)# * 1e3)
        self.g_ask_cnt[lon, lat] += 1.0
        ...
        # true all-sky O3:
        # (Keep for testing effect of thick clouds on cloud-sliced UT O3):
        if ( self.cld_frac_filter ):
            if np.sum(gc_data.t_cld_fr[gc_data.level_min:gc_data.level_max + 1, y, x]) < 0.7:
                return

        self.g_o3[lon][lat].append(gc_data.o3_2d)
        self.g_grad[lon][lat].append(gc_data.grad)
        self.strat_o3[lon][lat].append(gc_data.strat_col)
    ...
    def process_grid_square(self, i, j):
        ...
        t_col_o3 = np.asarray(self.g_o3[i][j],dtype=np.float)
        t_strat_o3 = np.asarray(self.strat_o3[i][j],dtype=np.float)
        t_fr_c = np.asarray(self.all_cld_fr[i][j],dtype=np.float)
        t_cld = np.asarray(self.all_cld_p[i][j],dtype=np.float)
        t_mr_o3 = np.asarray(self.g_true_o3[i][j],dtype=np.float)
        #t_o3 = np.asarray(self.g_o3[i][j],dtype=np.float)
        t_grad_o3 = np.asarray(self.g_grad[i][j],dtype=np.float)
        ...
        if (np.std(t_strat_o3) / np.mean(t_strat_o3)) > self.strat_filt:
        ...
        # Use cloud_slice_ut_o3 function to get cloud-sliced UT O3.
        # Treat data differently depending on whether there are 10-99 points:
        if n_pnts >= 20:# and n_pnts <100:
            self.add_slice(i,j,t_cld,t_col_o3, t_mr_o3, t_fr_c, t_grad_o3)#, t_o3)
        # Or at least 100 points:
            ...
            for w in nloop:
                subset_t_col_o3 = t_col_o3[w::stride]
                subset_t_cld = t_cld[w::stride]
                subset_t_mr_o3 = t_mr_o3[w::stride]
                subset_t_fr_c = t_fr_c[w::stride]
                subset_t_grad_o3 = t_grad_o3[w::stride]
                #subset_t_o3 = t_o3[w::stride]
                self.add_slice(i, j, subset_t_cld, subset_t_col_o3, subset_t_mr_o3, subset_t_fr_c, subset_t_grad_o3)#, subset_t_o3)

    def add_slice(self, i, j, t_cld, t_col_o3, t_mr_o3, t_fr_c, t_grad_o3):#, t_o3):
        ...
        :type t_cld: float
        :param t_col_o3: Total (stratosphere+troposphere) column o3
        :type t_col_o3: float
        :param t_mr_o3: O3 mixing ratio between pmax and pmin
        :type t_mr_o3: float
        :param t_fr_c: Cloud fraction between pmax and pmin
        :type t_fr_c: float
        :param t_grad_o3: O3 gradient between pmax and pmin in ppbv/hPa
        :type t_grad_o3: float
        """
        utmro3, utmro3err, stage_reached, mean_cld_pres = cldslice(t_col_o3, t_cld)
        ...
        if np.isnan(utmro3) or np.isnan(utmro3err):
            ...
            if ( stage_reached==0 ):
                print("Cloud-sliced O3 NAN for pixel i:{} j:{}".format(i, j))
                return
            self.loss_count[CLOUD_SLICE_ERROR_ENUM[stage_reached]] += 1
            # Track non-uniform O3 removed:
            grad_ind=np.where(np.abs(t_grad_o3) >= 0.33)[0]
            self.grad_remove += len(grad_ind)
            #print("Cloud-slice exception {} in pixel i:{} j:{}".format(
            #    CLOUD_SLICE_ERROR_ENUM[stage_reached], i, j))
        else:
            print('UT O3 cldslice, mean:', utmro3, np.mean(t_mr_o3),len(t_mr_o3))
            # Calculate weights:
            #err_wgt = 1.0 / (utmro3err ** 2)  # not use, but preserve anyway
            gaus_wgt = np.exp((-(mean_cld_pres - 315) ** 2) / (2 * 135 ** 2))
            
            # Get and track error range:
            nerr = utmro3err / utmro3
            if nerr > self.maxerr:
                self.maxerr = nerr
                print('Max rel. cloud-sliced O3 error: ', self.maxerr, flush=True)
            if nerr < self.minerr:
                self.minerr = nerr
                print('Min rel. cloud-sliced O3 error: ', self.minerr, flush=True)
                
            # Track non-uniform O3 retained:
            grad_ind=np.where(np.abs(t_grad_o3) >= 0.33)[0]
            self.grad_retain += len(grad_ind)
            
            # Weighted mean for each pass of cldslice:
            weight_mean = np.mean(t_mr_o3) * gaus_wgt 
            self.true_o3[i, j] += weight_mean
            #self.true_o3[i, j] += (np.mean(t_o3) * gaus_wgt)
            self.g_o3_vmr[i, j] += utmro3 * gaus_wgt
            self.g_slope_err[i, j] += utmro3err * gaus_wgt
    ...
    def get_weighted_mean(self):
        ...
        self.g_o3_vmr = np.divide(self.g_o3_vmr, self.g_gaus_wgt, where=self.g_cnt != 0)
        #self.g_o3_vmr = np.divide(self.g_o3_vmr, self.g_cnt, where=self.g_cnt != 0)
        ...
        #self.true_o3 = np.divide(self.true_o3, self.g_cnt, where=self.g_cnt != 0)
        self.g_cld_p = np.divide(self.g_cld_p, self.g_cnt, where=self.g_cnt != 0)
        #self.true_o3 = np.divide(self.true_o3, self.g_gaus_wgt, where=self.g_cnt != 0)
        self.g_askut_o3 = np.divide(self.g_askut_o3, self.g_ask_gaus_wgt, where=self.g_ask_cnt != 0)
        #self.g_askut_o3 = np.divide(self.g_askut_o3, self.g_ask_cnt, where=self.g_ask_cnt != 0)
        # Mean cloud-sliced UT O3 error:
        ...
        self.g_askut_o3[self.g_ask_cnt == 0] = np.nan
        self.g_o3_vmr[self.g_cnt == 0] = np.nan
    ...
    def print_data_report(self):
        ...
        print('(6) Outlier (O3 > 500 ppbv): ', self.loss_count["o3_outlier"], flush=True)
        ...
        print('Non-uniform O3 retained = ', self.grad_retain)
        print('Non-uniform O3 removed = ', self.grad_remove)
        print('Error range of cloud-sliced UT O3: ', np.nanmin(self.g_slope_err), np.nanmax(self.g_slope_err))
    ...
    def plot_data(self):
        ...
        plt.subplot(2, 3, 1)
        cs = m.pcolor(xi, yi, np.squeeze(self.g_o3_vmr), vmin=50, vmax=100, cmap='jet')
        ...
        plt.title('Cloud-sliced O3 VMRs')
        plt.subplot(2, 3, 2)
        cs = m.pcolor(xi, yi, np.squeeze(self.true_o3), vmin=50, vmax=100, cmap='jet')
        m.drawparallels(np.arange(-80., 81., 45.), labels=[1, 0, 0, 0], fontsize=8)
        m.drawmeridians(np.arange(-180., 181., 45.), labels=[0, 0, 0, 1], fontsize=8)
        m.drawcoastlines()
        m.drawcountries()
        cbar = m.colorbar(cs, location='bottom', pad="10%")
        plt.title('True cloudy O3')
        plt.subplot(2, 3, 3)
        cs = m.pcolor(xi, yi, np.squeeze(self.g_askut_o3), vmin=50, vmax=100, cmap='jet')
        m.drawparallels(np.arange(-80., 81., 45.), labels=[1, 0, 0, 0], fontsize=8)
        m.drawmeridians(np.arange(-180., 181., 45.), labels=[0, 0, 0, 1], fontsize=8)
        m.drawcoastlines()
        m.drawcountries()
        cbar = m.colorbar(cs, location='bottom', pad="10%")
        plt.title('True O3 VMRs under all-sky conditions')
        plt.subplot(2, 3, 4)
        plt.plot(self.true_o3, self.g_o3_vmr, 'o', color='black', markersize=6)
        r = stats.pearsonr(self.true_o3[~np.isnan(self.g_o3_vmr)],
                           self.g_o3_vmr[~np.isnan(self.g_o3_vmr)])
        # print('Correlation = ', r[0])
        result = rma(self.true_o3[~np.isnan(self.g_o3_vmr)], self.g_o3_vmr[~np.isnan(self.g_o3_vmr)],
                     len(self.true_o3[~np.isnan(self.g_o3_vmr)]), 1000)
        print(result, flush=True)
        xvals = np.arange(0, 120, 5)
        yvals = result[1] + xvals * result[0]
        plt.plot(xvals, yvals, '-')
        plt.xlim(40, 130)
        plt.ylim(40, 130)
        plt.xlabel('True O3 (cloudy)')
        plt.ylabel('Cloud-sliced O3')
        print('===== True (cloudy) vs cloud-sliced UT O3 ====')
        ...
        plt.text(48, 95, add2plt, fontsize=8,
                 ha='left', va='center')  # , transform=ax.transAxes)
        add2plt = ("R = {a:6.2f}".format(a=r[0]))
        plt.text(48, 85, add2plt, fontsize=8,
                 ha='left', va='center')  # , transform=ax.transAxes)
        plt.subplot(2, 3, 5)
        plt.plot(self.g_askut_o3, self.g_o3_vmr, 'o', color='black', markersize=6)
        r = stats.pearsonr(self.g_askut_o3[(~np.isnan(self.g_o3_vmr)) & (~np.isnan(self.g_askut_o3))],
                           self.g_o3_vmr[(~np.isnan(self.g_o3_vmr)) & (~np.isnan(self.g_askut_o3))])
        result = rma(self.g_askut_o3[(~np.isnan(self.g_o3_vmr)) & (~np.isnan(self.g_askut_o3))],
                     self.g_o3_vmr[(~np.isnan(self.g_o3_vmr)) & (~np.isnan(self.g_askut_o3))],
                     len(self.g_askut_o3[(~np.isnan(self.g_o3_vmr)) & (~np.isnan(self.g_askut_o3))]),
                     1000)
        xvals = np.arange(0, 120, 5)
        yvals = result[1] + xvals * result[0]
        plt.plot(xvals, yvals, '-')
        ...
        plt.text(48, 85, add2plt, fontsize=8, \
                 ha='left', va='center')  # , transform=ax.transAxes)
        plt.subplot(2, 3, 6)
        plt.plot(self.g_askut_o3, self.true_o3, 'o', color='black', markersize=6)
        r = stats.pearsonr(self.g_askut_o3[(~np.isnan(self.true_o3)) & (~np.isnan(self.g_askut_o3))],
                           self.true_o3[(~np.isnan(self.true_o3)) & (~np.isnan(self.g_askut_o3))])
        result = rma(self.g_askut_o3[(~np.isnan(self.true_o3)) & (~np.isnan(self.g_askut_o3))],
                     self.true_o3[(~np.isnan(self.true_o3)) & (~np.isnan(self.g_askut_o3))],
                     len(self.g_askut_o3[(~np.isnan(self.true_o3)) & (~np.isnan(self.g_askut_o3))]),
                     1000)
        xvals = np.arange(0, 120, 5)
        yvals = result[1] + xvals * result[0]
        plt.plot(xvals, yvals, '-')
        ...
        plt.ylim(40, 130)
        plt.xlabel('True O3 (all-sky)')
        plt.ylabel('True O3 (cloudy)')
        add2plt = ("y = {a:6.2f}x + {b:6.3f}".format(a=result[0], b=result[1]))
        plt.text(48, 95, add2plt, fontsize=8, ha='left', va='center')
        add2plt = ("R = {a:6.2f}".format(a=r[0]))
        plt.text(48, 85, add2plt, fontsize=8, ha='left', va='center')
        plt.show()
    ...
    def save_to_netcdf(self, out_path):
        ...
        # create data axes:
        # (1) Cloud-sliced O3:
        csuto3 = ncout.createVariable('csuto3', np.float32, ('lon', 'lat'))
        csuto3.units = 'ppbv'
        csuto3.long_name = 'UT O3 mixing ratio (180-450 hPa) obtained using cloud-slicing'
        csuto3[:] = self.g_o3_vmr
        # (2a) Double-weighting error:
        utdblerr = ncout.createVariable('utdblerr', np.float32, ('lon', 'lat'))
        utdblerr.units = 'ppbv'
        utdblerr.long_name = 'Standard error of the O3 mixing ratios in the UT (180-450 hPa) obtained using cloud-slicing'
        utdblerr[:] = self.g_slope_err
        # (2b) Gaussian-weighting error:
        utgauserr = ncout.createVariable('utgauserr', np.float32, ('lon', 'lat'))
        utgauserr.units = 'ppbv'
        utgauserr.long_name = 'Standard error of the O3 mixing ratios in the UT (180-450 hPa) obtained using cloud-slicing'
        utgauserr[:] = self.g_gaus_wgt
        # (3) Number of observations in each gridsquare:
        nobs = ncout.createVariable('nobs', np.float32, ('lon', 'lat'))
        nobs.units = 'unitless'
        nobs.long_name = 'Number of observations in each gridsquare used to obtain cloud-sliced UT O3 mixing ratios'
        nobs[:] = self.g_cnt
        ...
        utcld[:] = self.g_cld_p
        # (5) Mean O3 mixing ratio at 450-180 hPa for scenes with clouds:
        clduto3 = ncout.createVariable('clduto3', np.float32, ('lon', 'lat'))
        clduto3.units = 'ppbv'
        clduto3.long_name = 'UT O3 mixing ratio (180-450 hPa) obtained if clouds are present'
        clduto3[:] = self.true_o3
        # (6) Mean O3 mixing ratio at 450-180 hPa under all conditions (all-sky):
        askuto3 = ncout.createVariable('askuto3', np.float32, ('lon', 'lat'))
        askuto3.units = 'ppbv'
        askuto3.long_name = 'UT O3 mixing ratio (180-450 hPa) obtained under all conditions (all-sky)'
        askuto3[:] = self.g_askut_o3
        # (7) Cloud fraction:
        ...
        #uto3.units = 'ppbv'
        #uto3.long_name = 'GEOS-Chem ozone obtained coincident with cloud-sliced O3'
        #uto3[:] = self.true_o3
        # Close the file:
        ncout.close()
...
class GeosChemDay:
    ...
    def __init__(self, file_path, temperature_correction=False, cloud_height_test=False):
        ...
        tlon, tlat, tgco3, tcldfr, tcldhgt, tadn, tbxhgt, tpedge, tpause, tdegk = \
            fh.variables['LON'], fh.variables['LAT'], \
            fh.variables['IJ-AVG-S__O3'], fh.variables['TIME-SER__CF'], \
            fh.variables['TIME-SER__CThgt'], fh.variables['TIME-SER__AIRDEN'], \
            fh.variables['BXHGHT-S__BXHEIGHT'], fh.variables['PEDGE-S__PSURF'], \
            fh.variables['TR-PAUSE__TP-PRESS'], fh.variables['DAO-3D-S__TMPU']
        self.t_lon = tlon[:]
        self.t_lat = tlat[:]
        self.t_gc_o3 = tgco3[:]
    ...
    def prepare_o3_pixel(self, x, y):
        """Extracts preprocesed o3 from the geoschem pixel at x,y
        ...
        # "true' all-sky UT O3 and partial columns:
        lind = np.where((tp_mid >= P_MIN) & (tp_mid <= P_MAX))[0]
        # Get UT O3 under "true" all-sky conditions:
        ...
        self.temp_corr = np.ones(len(self.t_gc_o3[self.level_min:, y, x]))

        # Calculate O3 gradient:
        regr = LinearRegression()  
        # Pressure (hPa)
        x_vals=tp_mid[self.level_min:self.level_max].reshape(-1,1)
        # O3 (ppbv)
        y_vals=self.t_gc_o3[self.level_min:self.level_max, y, x].reshape(-1,1)
        ## O3 (ppbv)
        #y_vals=y_vals#*1e3
        # Perform regression:
        regr.fit(x_vals,y_vals)
        # Define gradient from regression slope (ppbv/hPa):
        self.grad = regr.coef_

        # Get partial O3 column in molec/m2 from cloud top height
        # to highest model level (output up to level 47):
        # print(t_gc_o3[self.level_min:tppind,y,x])
        # print(t_gc_o3[self.level_min:tppind,y,x]*1.5)
        self.o3_2d = np.sum(self.t_gc_o3[self.level_min:, y, x]
                             * 1e-5
                             * self.temp_corr
        ...
        self.strat_col = np.sum(self.t_gc_o3[tppind:, y, x]
...
if __name__ == "__main__":
    ...
    out_file = os.path.join(args.out_dir, 'gc-v12-1-0-ut-o3'
                            + '-' + args.region
                            + '-' + args.resolution
                            + '-jja-' + yrrange
                            + '-' + "gaus-wgt" )
    ...
    out_file += "-TS-v1-apr2021.nc"

    if float(args.strat_filter_threshold)>=10:
        strat_filter_threshold = float(args.strat_filter_threshold)/1000
    if float(args.strat_filter_threshold)<10:
