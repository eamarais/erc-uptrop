...
class CloudDataTropomiO3:
    """Class for containing the data for cloud filtering and analysis."""
    def __init__(self, file_path, fresco_440):
        ...
        self.get_cloud_fields(file_path)
    ...
    def get_cloud_fields(self, file_path):
        ...
        # Cloud top height in m (DLR-OCRA, so read height and convert to altitude):
        gcldhgt = fh.groups['PRODUCT']['SUPPORT_DATA']['INPUT_DATA']. \
                  variables['cloud_height_crb']
        tcldhgt = gcldhgt[0, :, :]

        # Define pressure array of zeros:
        self.tcldpres = np.zeros(tcldhgt.shape)

        # Convert cloud top altitude in m to cloud top pressure in Pa:
        hgtind = np.where(tcldhgt != fillval)
        self.tcldpres[hgtind] = alt2pres(tcldhgt[hgtind])
        #tcldpres[tcldhgt != fillval] = alt2pres(tcldhgt)
        
        # Get scene and surface pressure to diagnose clouds misclassified as snow/ice:
        # Apparent scene pressure:
        #gscenep = fh.groups['PRODUCT']['SUPPORT_DATA']['DETAILED_RESULTS']. \
        #              variables['scene_pressure'][:]
